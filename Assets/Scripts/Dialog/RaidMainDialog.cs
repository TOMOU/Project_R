...
namespace Dialog
{
    public class RaidMainDialog : IDialog
    {
        [Header("- Top")]
        ...
        [Header("- BossInfo")]
        [SerializeField] private Text _bossLevelText;
        [SerializeField] private List<Image> _bossHPValue;
        [SerializeField] private Text _bossHPCount;
        [SerializeField] private List<HpBar_BossPart> _bossPartHPBar;
        ...
        private UnitStatus _bossStatus;
        private int _curCount;
        private int _curIdx;
        private int _curHP;
        private Image _curFill;
        ...
        protected override void OnLoad()
        {
            ...
            Message.AddListener<BossInitMsg>(OnBossInit);
        }
        ...
        protected override void OnUnload()
        {
            ...
            Message.RemoveListener<BossInitMsg>(OnBossInit);
        }
        ...
        private void RefreshBossInfo()
        {
            ...
            if (_curHP > _bossStatus.hp)
            {
                _curHP -= speed;
            }
            else if (_curHP < _bossStatus.hp)
            {
                _curHP = _bossStatus.hp;
            }
            else if (_curHP < 0)
            {
                _curHP = 0;
            ...
            _curCount = _curHP / 1000;

            if (_curFill == null || _curIdx != _curCount % 7)
            {
                if (_curFill != null)
                    _curFill.fillAmount = 0f;

                _curIdx = _curCount % 7;
                _curFill = _bossHPValue[_curIdx];
                _curFill.transform.SetAsLastSibling();

                if (_bossStatus.hp > 1000)
                {
                    if (_curIdx == 0)
                    {
                        _bossHPValue[_bossHPValue.Count - 1].fillAmount = 1f;
                    }
                    else
                    {
                        _bossHPValue[_curIdx - 1].fillAmount = 1f;
                    }
                }

                _bossHPCount.text = string.Format("X{0}", _curCount);
            }

            _curFill.fillAmount = (_curHP % 1000) / 1000f;
        }
        ...
        {
            RequestDialogEnter<RaidPauseDialog>();
        }
        ...
        private void OnBossInit(BossInitMsg msg)
        {
            this._bossStatus = msg.Info.Status;
            _curHP = _bossStatus.hp;

            for (int i = 0; i < _bossPartHPBar.Count; i++)
            {
                _bossPartHPBar[i].Init(msg.Info);
            }
        }
        ...
        private void OnShowDamage(ShowDamageMsg msg)
        {
            ...
            if (d != null)
            {
                d.ActiveDamage(msg.target.transform, msg.damage, msg.isCritical, msg.slotIndex);
