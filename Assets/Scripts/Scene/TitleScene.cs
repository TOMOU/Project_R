...
namespace Scene
{
    public class TitleScene : IScene
    {
        ...
        protected override void OnLoadComplete()
        {
            ...
            // 초기에 보여줄 다이얼로그 활성화
            Dialog.IDialog.RequestDialogEnter<Dialog.TitleMainDialog>();
            Dialog.IDialog.RequestDialogEnter<Dialog.GlobalDebugDialog>();

            // 사운드 전부 끄고
            Message.Send<Global.StopAllSoundMsg>(new Global.StopAllSoundMsg());

            // BGM 재생
            Message.Send<Global.PlaySoundMsg>(new Global.PlaySoundMsg(Constant.SoundName.Title));
            // Message.Send<Global.PlaySoundMsg>(new Global.PlaySoundMsg(Constant.SoundName.Title));
        }
