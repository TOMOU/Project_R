...
namespace Logic
{
    public class BattleRaidLogic : ILogic
    {
        ...
        protected override void Initialize()
        {
            ...
            if (BattleManager.Singleton.battleType == 4 || (BattleManager.Singleton.battleType == 1 && BattleManager.Singleton.selectChapter == 4 && BattleManager.Singleton.selectStage == 1))
                _bgRoot.localPosition = new Vector3(-1.65f, 0f, 0f);
            else
        ...
        private void GameStart()
        {
            ...
            Dialog.IDialog.RequestDialogEnter<Dialog.RaidMainDialog>();
        ...
        private void SummonAllyLogic()
        {
            ...
            for (int i = 0; i < inven.bossTeam.Length; i++)
            {
                Info.Character cha = inven.GetCharacterByIndex(inven.bossTeam[i]);
                if (cha != null)
                {
                    var info = SummonAlly(cha.GetStatus(), Constant.Team.Blue, i);
                    if (info != null)
                    {
        ...
        private void ActiveEnemy()
        {
            ...
            for (int i = 0; i < r.objList.Count; i++)
            {
                ...
                if (r.statList[i].unitName == "j")
                {
                    // info.CheatHP(5);
                    Message.Send<Battle.Normal.BossInitMsg>(new Battle.Normal.BossInitMsg(info));
                }
        ...
        private void RedTeam_Position()
        {
            ...
            {
                if (info.Status.unitName == "j")
                {
                    info.transform.localPosition = new Vector3(10f, -13f, -9f);
                }
                else
                {
        ...
        private void BlueTeam_Position()
        {
            ...
            Vector3[] vecArr = { new Vector3(-14.5f, 2.3f, 2.3f), new Vector3(-10f, -0.4f, -0.4f), new Vector3(-15f, -3f, -3f), new Vector3(-7.5f, -5.5f, -5.5f), new Vector3(-12.3f, -8f, -8f) };

            for (int i = 0; i < list.Count; i++)
            {
                list[i].transform.localPosition = vecArr[i];
                list[i].InitBasePosition();
                list[i].transform.localPosition += Vector3.left * 18f;
            }

            // if (BattleManager.Singleton.battleType == 4)
            // {
            //     Vector3[] vecArr = { new Vector3(-13f, 0.7f, 0.7f), new Vector3(-6.5f, -1.1f, -1.1f), new Vector3(-10.5f, -3.3f, -3.3f), new Vector3(-6.5f, -5.8f, -5.8f), new Vector3(-11f, -7.5f, -7.5f) };
        ...
        private void Update()
        {
            ...
            if (_depth == Constant.BattlePhase.StartBattle && _enableInstantKill == true && TutorialManager.Singleton.IsTutorialProcess == false)
            {
                ...
                if (_maxTime - _curTime <= 30f && _curPhase == 3 && TutorialManager.Singleton.curTutorialIndex == 83)
                {
                    Action_Summon_J();
                }
        ...
        private void RefreshTimeInUI(UnityEngine.UI.Text text)
        {
            int remainTime = (int)(_maxTime - _curTime);
            text.text = string.Format("{0:D2}:{1:D2}", remainTime / 60, remainTime % 60);
        }
        ...
        private void StartBattle()
        {
            ...
            else if (_curPhase == 1 && TutorialManager.Singleton.curTutorialIndex == 89)
            {
        ...
        private void BattleEnd()
        {
            ...
            Dialog.IDialog.RequestDialogExit<Dialog.RaidMainDialog>();
            Dialog.IDialog.RequestDialogEnter<Dialog.RaidResultDialog>();
        ...
        private void ShowResultDialog_Draw()
        {
            Dialog.IDialog.RequestDialogExit<Dialog.RaidMainDialog>();
            Dialog.IDialog.RequestDialogEnter<Dialog.RaidResultDialog>();
            ...
            _coroutine = StartCoroutine(coShowResult_Draw());
        }

        private IEnumerator coShowResult_Draw()
        {
            yield return new WaitForSeconds(2f);
        ...
        {
            Dialog.IDialog.RequestDialogExit<Dialog.RaidMainDialog>();
            Dialog.IDialog.RequestDialogExit<Dialog.RaidResultDialog>();
        }
            ...
            list.ForEach(e =>
                {
                    for (int i = 0; i < 5; i++)
                    {
                        Message.Send<Battle.Normal.SendDamageMsg>(new Battle.Normal.SendDamageMsg(e, 99999, true, i));
                        Message.Send<Battle.Normal.SendDamageMsg>(new Battle.Normal.SendDamageMsg(e, 99999, true, i));
                    }
                });
        }
        ...
        //     {
        //         if (GUI.Button(new Rect(30, 100, 100, 100), "시나리오 강제진입"))
        //         {
        //             _curTime = _maxTime - 30f;
        //         }
        //     }
        //     else if (GUI.Button(new Rect(30, 100, 100, 100), "몬스터 일격사"))
        //     {
